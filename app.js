(() => {
  ...
  const instagramProfile = value => {
    ...
    if (!raw) return null;
    const match = raw.match(/^(?:https?:\/\/)?(?:www\.)?instagram\.com\/([A-Za-z0-9._]{1,30})\/?(?:[?#].*)?$/i);
    const username = (match ? match[1] : raw.replace(/^@/, '')).trim();
    if (!/^[A-Za-z0-9._]{1,30}$/.test(username)) return null;
    return {username, url:`https://www.instagram.com/${encodeURIComponent(username)}/`};
  };
  const defaultRegistrationSettings = () => ({
    codeUrl:'https://docs.google.com/document/d/1uq8BiFuCNeebo8rMKxH6691JDzvYAvtz3sdLm5DGaEA/edit?tab=t.0',
    statementUrl:'https://docs.google.com/document/d/1Z9fdJZiKVHMExj4plYXKBTjfEM1XoL1ANyU3CZ-CE9o/edit?tab=t.0',
    scheduleUrl:'', scheduleText:''
  });
  const safeExternalUrl = value => /^https?:\/\//i.test(String(value ?? '').trim()) ? String(value).trim() : '';
  ...
  const newEvent = (overrides = {}) => ({
    ...
    registration:clone({...defaultRegistrationSettings(), ...(overrides.registration || {})}),
  ...
  function ensureEvent(event) {
    ...
    const defaults = defaultRegistrationSettings(); const configuredRegistration = event.registration || {};
    event.registration = {...defaults, ...configuredRegistration};
    event.registration.codeUrl = configuredRegistration.codeUrl === undefined ? defaults.codeUrl : safeExternalUrl(configuredRegistration.codeUrl);
    event.registration.statementUrl = configuredRegistration.statementUrl === undefined ? defaults.statementUrl : safeExternalUrl(configuredRegistration.statementUrl);
    event.registration.scheduleUrl = safeExternalUrl(event.registration.scheduleUrl);
    event.registration.scheduleText = String(event.registration.scheduleText || '');
  ...
  function renderEvents() {
    const eventCard = event => `<article class="card event-card"><div class="event-card-top"><span class="pill ${statusClass(event.status)}">${escapeHtml(statusLabel(event.status))}</span><span class="muted">${escapeHtml(dateLabel(event.date))}</span></div><h2>${escapeHtml(event.name)}</h2><p class="muted">${escapeHtml(event.location || 'Místo bude doplněno')}</p>${event.info ? `<p>${escapeHtml(event.info)}</p>` : ''}<div class="event-card-meta"><span>${event.riders.length} jezdců</span><span>${event.categories.length} kategorií</span><span>${event.judgeCount} porotci</span></div><div class="toolbar"><button class="btn btn-primary" data-open-event="${escapeHtml(event.id)}">Otevřít závod</button>${event.status !== 'finished' ? `<button class="btn btn-outline" data-register-event="${escapeHtml(event.id)}">Přihlásit se</button>` : ''}</div></article>`;
    const sortEvents = events => events.slice().sort((a,b) => String(a.date).localeCompare(String(b.date)));
    const activeEvents = sortEvents(state.events.filter(event => event.status !== 'finished'));
    const completedEvents = sortEvents(state.events.filter(event => event.status === 'finished'));
    const section = (title, events, emptyText) => `<section class="events-section"><div class="section-title"><div><h2>${title}</h2><p>${title === 'Probíhající závody' ? 'Vyber závod, který chceš zobrazit.' : 'Závody, které jsou uzavřené a dokončené.'}</p></div><span class="pill">${events.length}</span></div><div class="event-grid">${events.map(eventCard).join('') || `<div class="card empty">${emptyText}</div>`}</div></section>`;
    app.innerHTML = publicHero('Závody.', 'Každý závod má vlastní datum, informace, porotu, kategorie, heaty a výsledky.') + section('Probíhající závody', activeEvents, 'Zatím není vytvořený žádný probíhající závod.') + (completedEvents.length ? section('Dokončené závody', completedEvents, '') : '');
    $$('[data-open-event]').forEach(button => button.onclick = () => setActiveEvent(button.dataset.openEvent, 'live'));
    $$('[data-register-event]').forEach(button => button.onclick = () => setActiveEvent(button.dataset.registerEvent, 'registration'));
  }
  ...
  function renderLive() {
    ...
    app.innerHTML = publicHero('Výsledky závodu.', 'U každého porotce se použije lepší jízda. Výsledek čeká na kompletní hodnocení panelu.') + `<div class="section-title"><div><h2>Live leaderboard</h2><p>${escapeHtml(ruleText(event))} • ${event.runCount} ${event.runCount === 1 ? 'jízda' : 'jízdy'}</p></div><span class="pill">${DEMO ? 'DEMO' : 'ONLINE'}</span></div><div class="toolbar"><div class="tabs">${categories.map(category => `<button class="tab ${category.id === selected ? 'active' : ''}" data-live-cat="${escapeHtml(category.id)}">${escapeHtml(category.name)}</button>`).join('')}</div><button class="btn btn-outline" data-action="export-results">Export Excel</button></div><div class="table-wrap"><table class="table"><thead><tr><th>#</th><th>Jezdec</th>${panel(event).map((id,index) => `<th title="${escapeHtml(event.judges.find(judge => judge.id === id)?.name || '')}">Porotce ${index + 1}</th>`).join('')}<th>Výsledek</th></tr></thead><tbody>${rows.map(row => { const instagram = instagramProfile(row.instagram); return `<tr><td class="rank">${row.rank ?? '—'}</td><td><div class="rider-name">${escapeHtml(row.name)}</div>${instagram ? `<a class="rider-instagram" href="${escapeHtml(instagram.url)}" target="_blank" rel="noopener noreferrer">@${escapeHtml(instagram.username)}</a>` : ''}<div class="rider-meta">#${row.bib} • ${row.birthYear || 'rok neuveden'}${row.heat ? ` • Heat ${row.heat}` : ''}</div></td>${row.marks.map((value,index) => `<td class="score">${value == null ? '—' : row.dropped.includes(index) ? `<del title="Škrtnutá známka">${value.toFixed(2)}</del>` : value.toFixed(2)}</td>`).join('')}<td class="score score-best">${row.total == null ? `<span class="help">Čeká ${row.received}/${event.judgeCount}</span>` : row.total.toFixed(2)}</td></tr>`; }).join('') || `<tr><td colspan="${panel(event).length + 3}" class="empty">Zatím bez výsledků.</td></tr>`}</tbody></table></div><p class="help">Přeškrtnuté známky se nepočítají. Při shodě se pořadí sdílí.</p>`;
  ...
  function renderStartlist() {
    const openEvents = state.events.filter(item => item.status !== 'finished'); const event = openEvents.find(item => item.id === state.activeEventId) || openEvents[0] || activeEvent(); if (event.id !== state.activeEventId) { state.activeEventId = event.id; save(); }
    const categories = eventCategories(event); const selected = sessionStorage.getItem(`startCat:${event.id}`) || categories[0]?.id || '';
    const list = event.riders.filter(rider => rider.categoryId === selected).sort((a,b) => (a.heat || 999) - (b.heat || 999) || a.bib - b.bib);
    const eventOptions = openEvents.map(item => `<option value="${escapeHtml(item.id)}" ${item.id === event.id ? 'selected' : ''}>${escapeHtml(item.name)}${item.date ? ` • ${escapeHtml(dateLabel(item.date))}` : ''}</option>`).join('');
    app.innerHTML = publicHero('Startovní listina.', 'Přehled jezdců, ročníků, kategorií a ručně připravených heatů.') + `<div class="section-title"><div><h2>Startovka</h2><p>${list.length} jezdců • ${escapeHtml(categoryName(event, selected))}</p></div></div><div class="toolbar"><label class="inline-field">Závod<select id="startEvent" class="select">${eventOptions || `<option value="${escapeHtml(event.id)}" selected>${escapeHtml(event.name)}</option>`}</select></label><div class="tabs">${categories.map(category => `<button class="tab ${category.id === selected ? 'active' : ''}" data-start-cat="${escapeHtml(category.id)}">${escapeHtml(category.name)}</button>`).join('')}</div></div><div class="table-wrap"><table class="table"><thead><tr><th>Pořadí</th><th>#</th><th>Jezdec</th><th>Ročník</th><th>Heat</th><th>Stav</th></tr></thead><tbody>${list.map((rider,index) => `<tr><td>${index + 1}</td><td class="score">${rider.bib}</td><td class="rider-name">${escapeHtml(rider.name)}</td><td>${rider.birthYear || '—'}</td><td><span class="pill">${rider.heat ? `Heat ${rider.heat}` : 'Nepřiřazen'}</span></td><td><span class="pill ${rider.status === 'checked-in' ? 'live' : 'warn'}">${rider.status === 'checked-in' ? 'Prezentován' : 'Přihlášen'}</span></td></tr>`).join('')}</tbody></table></div>`;
    $('#startEvent')?.addEventListener('change', eventTarget => { const next = state.events.find(item => item.id === eventTarget.target.value && item.status !== 'finished'); if (!next) return; state.activeEventId = next.id; save(); renderStartlist(); });
  ...
  function renderRegistration() {
    const event = activeEvent();
    const settings = event.registration || defaultRegistrationSettings();
    const eventOptions = state.events.slice().sort((a,b) => String(a.date).localeCompare(String(b.date))).map(item => `<option value="${escapeHtml(item.id)}" ${item.id === event.id ? 'selected' : ''}>${escapeHtml(item.name)}${item.date ? ` • ${escapeHtml(dateLabel(item.date))}` : ''}</option>`).join('');
    const codeUrl = safeExternalUrl(settings.codeUrl); const statementUrl = safeExternalUrl(settings.statementUrl); const scheduleUrl = safeExternalUrl(settings.scheduleUrl);
    const links = [codeUrl ? `<a href="${escapeHtml(codeUrl)}" target="_blank" rel="noopener noreferrer">Kodex závodníka</a>` : '', statementUrl ? `<a href="${escapeHtml(statementUrl)}" target="_blank" rel="noopener noreferrer">Prohlášení účastníka</a>` : '', scheduleUrl ? `<a href="${escapeHtml(scheduleUrl)}" target="_blank" rel="noopener noreferrer">Harmonogram</a>` : ''].filter(Boolean).join('');
    const scheduleText = String(settings.scheduleText || '').trim();
    const conditions = links || scheduleText ? `<div class="registration-conditions field full"><b>Podmínky a informace k závodu</b>${links ? `<div class="registration-links">${links}</div>` : ''}${scheduleText ? `<div class="registration-schedule"><span>Harmonogram</span><p>${escapeHtml(scheduleText).replace(/\r?\n/g, '<br>')}</p></div>` : ''}${statementUrl ? `<label class="check-field"><input type="checkbox" name="termsAccepted" required> Souhlasím s kodexem závodníka a prohlášením účastníka.</label>` : ''}</div>` : '';
    app.innerHTML = `<div class="registration-shell"><div class="card"><div class="registration-head"><div><div class="eyebrow">PŘIHLÁŠKA NA ZÁVOD</div><h1>${escapeHtml(event.name)}</h1><p class="muted">Vyber závod a vyplň přihlášku. Kategorie se přidělí automaticky podle roku narození.</p></div><span class="pill">${escapeHtml(dateLabel(event.date))}</span></div><form id="registrationForm" class="form-grid"><div class="field full"><label for="registrationEvent">Závod *</label><select id="registrationEvent" class="select" required>${eventOptions}</select></div><div class="field full"><label>Jméno a příjmení *</label><input class="input" name="name" required placeholder="Jan Novák"></div><div class="field"><label>Rok narození *</label><input class="input" type="number" name="birthYear" min="1900" max="${yearNow + 1}" required placeholder="2012"></div><div class="field"><label>Město</label><input class="input" name="city" placeholder="Pardubice"></div><div class="field"><label>Instagram</label><input class="input" name="instagram" placeholder="@username"></div><div class="field full"><label>Sponzoři</label><input class="input" name="sponsors" placeholder="Divine, Scootshop..."></div><div class="field full"><label>Informace pro speakera</label><textarea class="textarea" rows="4" name="bio" placeholder="Krátké info o jezdci..."></textarea></div>${conditions}<div class="field full"><button class="btn btn-primary" type="submit">Odeslat přihlášku</button><div class="help">Zařazení: ${eventCategories(event).map(category => `${escapeHtml(category.name)} (${escapeHtml(categoryRange(category))})`).join(' • ') || 'kategorie zatím nejsou nastavené'}</div></div></form></div></div>`;
    $('#registrationEvent').onchange = eventTarget => { const next = state.events.find(item => item.id === eventTarget.target.value); if (!next) return; state.activeEventId = next.id; state.judgeState = {categoryId:'', heat:1, riderIndex:0, run:1, values:{}}; state.speakerState = {categoryId:'', heat:0, riderIndex:0}; save(); renderRegistration(); };
    $('#registrationForm').onsubmit = async formEvent => {
      ...
      if (statementUrl && values.termsAccepted !== 'on') { toast('Potvrď souhlas s prohlášením účastníka.'); return; }
  ...
  function adminEvents() {
    ...
    const eventCard = event => `<article class="card event-card ${event.id === active.id ? 'selected' : ''}"><div class="event-card-top"><span class="pill ${statusClass(event.status)}">${escapeHtml(statusLabel(event.status))}</span><span>${escapeHtml(dateLabel(event.date))}</span></div><h2>${escapeHtml(event.name)}</h2><p class="muted">${escapeHtml(event.location || 'Místo není nastavené')}</p><p>${escapeHtml(event.info || 'Bez popisu')}</p><div class="event-card-meta"><span>${event.judgeCount} porotci</span><span>${event.runCount} ${event.runCount === 1 ? 'jízda' : 'jízdy'}</span><span>${event.riders.length} jezdců</span></div><div class="toolbar"><button class="btn btn-primary" data-select-admin-event="${escapeHtml(event.id)}">${event.id === active.id ? 'Aktivní závod' : 'Otevřít'}</button><button class="btn btn-outline" data-edit-event-id="${escapeHtml(event.id)}">Upravit</button>${state.events.length > 1 ? `<button class="btn btn-danger" data-delete-event="${escapeHtml(event.id)}">Smazat</button>` : ''}</div></article>`;
    const sortEvents = events => events.slice().sort((a,b) => String(a.date).localeCompare(String(b.date)));
    const activeEvents = sortEvents(state.events.filter(event => event.status !== 'finished'));
    const completedEvents = sortEvents(state.events.filter(event => event.status === 'finished'));
    const section = (title, events, emptyText) => `<section class="events-section"><div class="section-title"><div><h2>${title}</h2><p>${title === 'Probíhající závody' ? 'Závody, které se připravují, probíhají nebo přijímají přihlášky.' : 'Uzavřené závody s hotovými výsledky.'}</p></div><span class="pill">${events.length}</span></div><div class="event-grid admin-event-grid">${events.map(eventCard).join('') || `<div class="card empty">${emptyText}</div>`}</div></section>`;
    const content = adminHeader('Závody', 'Vytvoř samostatný závod a nastav ho podle jeho potřeb.', `<button class="btn btn-primary" data-action="new-event">+ Nový závod</button>`) + section('Probíhající závody', activeEvents, 'Zatím není vytvořený žádný probíhající závod.') + section('Dokončené závody', completedEvents, 'Zatím není žádný dokončený závod.');
  ...
  function adminRiders() {
    const event = activeEvent();
    const riders = event.riders.slice().sort((a,b) => a.bib - b.bib);
    const content = adminHeader('Jezdci', 'Přihlášky, automatické zařazení podle ročníku a prezence.', `<button class="btn btn-outline" data-action="reassign">Přerozdělit podle ročníku</button><button class="btn btn-outline" data-action="import-csv">Import CSV</button><button class="btn btn-primary" data-action="add-rider">+ Přidat jezdce</button>`) + `<div class="table-wrap"><table class="table"><thead><tr><th>#</th><th>Jezdec</th><th>Ročník</th><th>Kategorie</th><th>Heat</th><th>Stav</th><th>Akce</th></tr></thead><tbody>${riders.map(rider => `<tr><td class="score">${rider.bib}</td><td><div class="rider-name">${escapeHtml(rider.name)}</div><div class="rider-meta">${escapeHtml(rider.city || '')}</div></td><td>${rider.birthYear || '—'}</td><td><span class="pill">${escapeHtml(categoryName(event, rider.categoryId))}</span></td><td>${rider.heat ? `Heat ${rider.heat}` : '—'}</td><td><button class="btn btn-small ${rider.status === 'checked-in' ? 'btn-primary' : 'btn-outline'}" data-checkin="${escapeHtml(rider.id)}">${rider.status === 'checked-in' ? 'Prezentován' : 'Prezentovat'}</button></td><td><button class="btn btn-small btn-outline" data-edit-rider="${escapeHtml(rider.id)}">Upravit</button> <button class="btn btn-small btn-danger" data-delete-rider="${escapeHtml(rider.id)}">Smazat</button></td></tr>`).join('')}</tbody></table></div>`;
    ...
    $$('[data-edit-rider]').forEach(button => button.onclick = () => openRiderForm(button.dataset.editRider));
    $$('[data-delete-rider]').forEach(button => button.onclick = () => { const rider = event.riders.find(item => item.id === button.dataset.deleteRider); if (!rider) return; const hasScores = event.scores.some(score => score.riderId === rider.id); const warning = hasScores ? `Smazat jezdce „${rider.name}“? Smažou se také jeho hodnocení.` : `Smazat jezdce „${rider.name}“?`; if (!confirm(warning)) return; event.riders = event.riders.filter(item => item.id !== rider.id); event.scores = event.scores.filter(score => score.riderId !== rider.id); save(); toast('Jezdec byl smazán.'); adminRiders(); });
  ...
  function openEventForm(id) {
    ...
    const settings = event.registration || defaultRegistrationSettings();
    modal(id ? 'Upravit závod' : 'Nový závod', `<form id="eventForm" class="form-grid"><div class="field full"><label>Název závodu *</label><input class="input" name="name" required value="${escapeHtml(event.name)}"></div><div class="field"><label>Datum závodu</label><input class="input" type="date" name="date" value="${escapeHtml(event.date)}"></div><div class="field"><label>Místo</label><input class="input" name="location" value="${escapeHtml(event.location)}"></div><div class="field"><label>Typ poroty</label><select class="select" name="judgeCount"><option value="3" ${event.judgeCount === 3 ? 'selected' : ''}>3 porotci</option><option value="5" ${event.judgeCount === 5 ? 'selected' : ''}>5 porotců</option></select></div><div class="field"><label>Počet jízd</label><select class="select" name="runCount">${[1,2,3].map(count => `<option value="${count}" ${event.runCount === count ? 'selected' : ''}>${count} ${count === 1 ? 'jízda' : 'jízdy'}</option>`).join('')}</select><div class="help">Závod nemá semifinále ani finále.</div></div><div class="field"><label>Stav závodu</label><select class="select" name="status">${['draft','registration','live','finished'].map(status => `<option value="${status}" ${event.status === status ? 'selected' : ''}>${statusLabel(status)}</option>`).join('')}</select></div><div class="field full"><label>Info o závodu</label><textarea class="textarea" name="info" rows="4" placeholder="Místo, čas, pravidla, kontakt...">${escapeHtml(event.info)}</textarea></div><div class="field full"><div class="section-title"><div><h3>Podmínky a harmonogram pro přihlášku</h3><p>Odkazy a text se zobrazí podle vybraného závodu.</p></div></div></div><div class="field full"><label>Kodex závodníka – odkaz</label><input class="input" type="url" name="codeUrl" value="${escapeHtml(settings.codeUrl || '')}" placeholder="https://..."></div><div class="field full"><label>Prohlášení účastníka – odkaz</label><input class="input" type="url" name="statementUrl" value="${escapeHtml(settings.statementUrl || '')}" placeholder="https://..."></div><div class="field full"><label>Harmonogram – odkaz</label><input class="input" type="url" name="scheduleUrl" value="${escapeHtml(settings.scheduleUrl || '')}" placeholder="https://... nebo odkaz na PDF"></div><div class="field full"><label>Harmonogram závodu</label><textarea class="textarea" name="scheduleText" rows="6" placeholder="08:00 Prezence\n09:00 Heat 1\n...">${escapeHtml(settings.scheduleText || '')}</textarea><div class="help">Můžeš vložit vlastní rozpis po řádcích. Zobrazí se přímo v přihlášce.</div></div><div class="field full"><button class="btn btn-primary">Uložit závod</button></div></form>`);
    $('#eventForm').onsubmit = formEvent => {
      formEvent.preventDefault(); const values = Object.fromEntries(new FormData(formEvent.currentTarget)); const registration = {codeUrl:safeExternalUrl(values.codeUrl), statementUrl:safeExternalUrl(values.statementUrl), scheduleUrl:safeExternalUrl(values.scheduleUrl), scheduleText:String(values.scheduleText || '').trim()};
      if (!id) { const created = newEvent({...values, judgeCount:Number(values.judgeCount), runCount:Number(values.runCount), registration}); ensureEvent(created); state.events.push(created); state.activeEventId = created.id; } else { Object.assign(event, {name:String(values.name).trim(), date:values.date, location:values.location, info:values.info || '', status:values.status, judgeCount:Number(values.judgeCount), runCount:Number(values.runCount), registration}); ensureEvent(event); }
      save(); closeModal(); toast('Závod je uložený.'); renderAdmin();
    };
  }
  ...
  function openRiderForm(id) {
    const event = activeEvent(); const existing = id ? event.riders.find(item => item.id === id) : null; if (id && !existing) return;
    const rider = existing || {id:uid('rider'), registrationOrder:event.riders.length + 1, name:'', birthYear:'', bib:Math.max(0, ...event.riders.map(item => Number(item.bib) || 0)) + 1, city:'', instagram:'', sponsors:'', bio:'', status:'registered'};
    modal(existing ? 'Upravit jezdce' : 'Přidat jezdce', `<form id="riderForm" class="form-grid"><div class="field full"><label>Jméno *</label><input class="input" name="name" required value="${escapeHtml(rider.name || '')}"></div><div class="field"><label>Rok narození *</label><input class="input" type="number" name="birthYear" min="1900" max="${yearNow + 1}" required value="${escapeHtml(rider.birthYear || '')}"></div><div class="field"><label>Startovní číslo</label><input class="input" type="number" name="bib" min="1" value="${escapeHtml(rider.bib || '')}"></div><div class="field"><label>Město</label><input class="input" name="city" value="${escapeHtml(rider.city || '')}"></div><div class="field"><label>Instagram</label><input class="input" name="instagram" value="${escapeHtml(rider.instagram || '')}" placeholder="@username"></div><div class="field"><label>Sponzoři</label><input class="input" name="sponsors" value="${escapeHtml(rider.sponsors || '')}"></div><div class="field"><label>Stav prezence</label><select class="select" name="status"><option value="registered" ${rider.status !== 'checked-in' ? 'selected' : ''}>Přihlášen</option><option value="checked-in" ${rider.status === 'checked-in' ? 'selected' : ''}>Prezentován</option></select></div><div class="field full"><label>Informace pro speakera</label><textarea class="textarea" name="bio" rows="3">${escapeHtml(rider.bio || '')}</textarea></div><div class="field full"><button class="btn btn-primary">Uložit jezdce</button></div></form>`);
    $('#riderForm').onsubmit = formEvent => {
      formEvent.preventDefault(); const values = Object.fromEntries(new FormData(formEvent.currentTarget)); const name = String(values.name || '').trim(); const birthYear = parseYear(values.birthYear); const bib = Number(values.bib);
      if (!name || !birthYear) { toast('Vyplň jméno a platný rok narození.'); return; }
      if (!Number.isInteger(bib) || bib < 1) { toast('Startovní číslo musí být kladné celé číslo.'); return; }
      if (event.riders.some(item => item.id !== id && Number(item.bib) === bib)) { toast(`Startovní číslo ${bib} už používá jiný jezdec.`); return; }
      Object.assign(rider, {name, birthYear, bib, city:String(values.city || '').trim(), instagram:String(values.instagram || '').trim(), sponsors:String(values.sponsors || '').trim(), bio:String(values.bio || '').trim(), status:values.status === 'checked-in' ? 'checked-in' : 'registered'});
      assignCategory(event, rider); if (!existing) event.riders.push(rider); save(); closeModal(); toast(existing ? 'Jezdec byl upraven.' : 'Jezdec byl přidán.'); adminRiders();
    };
  }
  ...
  function exportResults(event, categoryId) { const rows = [['Pořadí','Startovní číslo','Jezdec','Ročník','Kategorie',...panel(event).map((_,index) => `Porotce ${index + 1}`),'Výsledek','Hodnoceno','Porota'], ...leaderboard(event, categoryId).map(row => [row.rank,row.bib,row.name,row.birthYear,categoryName(event,row.categoryId),...row.marks,row.total == null ? '' : row.total.toFixed(2),row.received,event.judgeCount])]; if (!window.XLSX) { toast('Export Excel není dostupný. Obnov stránku a zkus to znovu.'); return; } const worksheet = window.XLSX.utils.aoa_to_sheet(rows); worksheet['!cols'] = rows[0].map((_,index) => ({wch:index === 2 ? 28 : 14})); const workbook = window.XLSX.utils.book_new(); window.XLSX.utils.book_append_sheet(workbook, worksheet, 'Výsledky'); const safeName = String(categoryName(event, categoryId)).normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '') || 'zavod'; window.XLSX.writeFile(workbook, `vysledky-${safeName}.xlsx`); }
