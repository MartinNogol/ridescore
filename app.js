(() => {
  ...
  function renderEvents() {
    const cards = state.events.slice().sort((a,b) => String(a.date).localeCompare(String(b.date))).map(event => `<article class="card event-card"><div class="event-card-top"><span class="pill ${statusClass(event.status)}">${escapeHtml(statusLabel(event.status))}</span><span class="muted">${escapeHtml(dateLabel(event.date))}</span></div><h2>${escapeHtml(event.name)}</h2><p class="muted">${escapeHtml(event.location || 'Místo bude doplněno')}</p>${event.info ? `<p>${escapeHtml(event.info)}</p>` : ''}<div class="event-card-meta"><span>${event.riders.length} jezdců</span><span>${event.categories.length} kategorií</span><span>${event.judgeCount} porotci</span></div><div class="toolbar"><button class="btn btn-primary" data-open-event="${escapeHtml(event.id)}">Otevřít závod</button><button class="btn btn-outline" data-register-event="${escapeHtml(event.id)}">Přihlásit se</button></div></article>`).join('');
